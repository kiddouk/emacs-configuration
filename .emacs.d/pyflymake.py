...
class LintRunner(object):
    ...
    def run(self, filename):
        ...
        logging.debug("read output")
        for line in getattr(process, self.stream):
            self.process_output(line)

        logging.debug("flush outputs")
...
class TestRunner(LintRunner):
    ...
    def run(self, filename):
        if "test" not in filename:
            ...
            (base, filetype) = base.split(".")
            base += "tests"
            test_file = "%s/%s.%s" % (file_dir, base, filetype)
            # call the nosetest on the file itself
            if os.path.exists(test_file) and os.path.isfile(test_file):
                print test_file
                super(TestRunner, self).run(test_file)
        super(TestRunner, self).run(filename)
